...
export function renderEstoque() {
    ...
    const estoqueFiltrado = mockData.estoque.filter(item => 
        item.nome.toLowerCase().includes(termoBusca) || 
        item.categoria.toLowerCase().includes(termoBusca) ||
        item.id.toLowerCase().includes(termoBusca)
    );
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="p-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <button class="p-1.5 text-green-600 hover:bg-green-50 rounded-lg transition-colors border border-transparent hover:border-green-200" title="Adicionar Entrada" onclick="window.abrirMovimentacaoModal('${item.id}', 'entrada')">
                                            <i data-lucide="arrow-down-to-line" class="w-4 h-4"></i>
                                        </button>
                                        <button class="p-1.5 text-orange-600 hover:bg-orange-50 rounded-lg transition-colors border border-transparent hover:border-orange-200" title="Registar Saída" onclick="window.abrirMovimentacaoModal('${item.id}', 'saida')">
